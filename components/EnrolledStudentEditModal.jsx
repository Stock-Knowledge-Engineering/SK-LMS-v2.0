...
export default function EnrolledStudentEditModal(props){
    const [subject, setSubject] = useState(props.studentSchedule.subject);
    const [student, setStudent] = useState(props.studentSchedule.studentid);
    const [section, setSection] = useState(props.studentSchedule.section);
    const [gradelevel, setGradeLevel] = useState(props.studentSchedule.gradelevel);
    ...
    const [gradelevelsLoading, gradelevels] = useHttp(
      ...
      const [isLoading, data] = usePostHttp(toSubmit ? {classSchedules: classSchedules, student: student } : null, `/student-schedule/edit`)
             ...
             <div className="p-12 absolute w-full m-auto h-full">
                <div className="border">
                    ...
                    <div className="bg-white p-4 flex flex-wrap justify-around content-evenly">
                        <Select setValue={setSubject} options={subjects} selected={props.studentSchedule.subject} value="subject" label={'subject'}/>   
                        <Select setValue={setGradeLevel} options={gradelevels} selected={props.studentSchedule.gradelevel} value="gradelevel" label={'gradelevel'}/>   
                        <Select setValue={setSection} options={sections} selected={props.studentSchedule.section} value="section" label={'section'}/>   
                        <Select setValue={setStudent} options={students} selected={props.studentSchedule.student_fullname} value="id" label={'fullname'}/>
                    </div>
