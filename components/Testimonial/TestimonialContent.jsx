            ...
            <div key={testimonial.id} className={`${testimonial.id == current ? 'block animate__animated animate__fadeIn' :'hidden'} h-auto w-full mx-4 my-12 sm:px-8 xs:p-2 space-y-2 rounded-xl`}>
              <div className="w-full flex space-x-3 xs:flex-col">
                ...
                <div className="w-full h-16">
                  <h6 className="lg:text-xl xs:text-sm font-bold text-blue-500">
                    {testimonial.name}
                  </h6>
                  <p className="text-blue-300 text-sm xs:text-xs">
                    {testimonial.description}
                  </p>
                </div>
              </div>
              <p className="leading-loose text-gray-400 lg:text-lg md:text-sm xs:text-sm">
                {`"${testimonial.message}"`}
              </p>
              {
