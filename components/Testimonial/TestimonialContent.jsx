const TestimonialContent = (props) => {
  ...
  return (
    ...
    {testimonials && testimonials.map((testimonial, id) => {
      return(
        <div key={id} className={`bg-white lg:h-auto md:h-auto reno:h-72 sm:h-auto xs:h-auto w-full mx-4 px-8 py-4 rounded-xl shadow-lg ${id == 0 ? 'mb-4' : ''}`}>
          ...
          <p className="text-blue-300 lg:text-xl md:text-base reno:text-base sm:text-xs xs:text-xs leading-tight">{testimonial.description}</p>
        </div>
      </div>
